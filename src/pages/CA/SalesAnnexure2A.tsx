// src/pages/CA/SalesAnnexure2A.tsx - Sales Return Register (Annexure 2A)
...
import { SalesReturnRecord, GSTSummary } from "../../types/caReports";
import ExcelJS from "exceljs";

type ViewMode = "customerwise" | "productwise" | "detailed";

export default function SalesAnnexure2A() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const data = await getSalesReturnRecords(filters);
      setRecords(data);
      setGstSummary(calculateGSTSummary(data));

      if (data.length === 0) {
        toast("No sales return records found for selected period", { icon: "ℹ️" });
      } else {
        toast.success(`Loaded ${data.length} sales return records`);
      }
    } catch (error) {
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      summarySheet.addRows([
        ...
        { particulars: "Total Return Value", amount: (gstSummary?.totalInvoiceValue ?? 0).toFixed(2) },
      ]);

      // Sheet 2: Detailed Records
      const detailSheet = workbook.addWorksheet("Sales Return Register");
      detailSheet.columns = [
      ...
      link.href = url;
      link.download = `Sales_Return_Annexure_2A_${dateFrom}_to_${dateTo}.xlsx`;
      link.click();
  ...
  return (
    <>
      <PageMeta
        title="Sales Annexure 2A"
        description="Customer-wise & Product-wise Sales Return Register"
      />

      <div className="grid grid-cols-12 gap-6">
        <div className="col-span-12">
          <TASection
            title="📋 Sales Annexure 2A - Sales Return Register"
            subtitle="GST-Compliant Sales Return Register with CGST, SGST, IGST Breakdowns"
          >
            {/* Filters */}
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl">
              <div>
                    ...
                    <tbody>
                      {records.map((record, index) => (
                        ...
                        >
                          ...
                          <td className="p-3 text-right font-mono font-semibold">₹{(record.totalValue ?? 0).toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
